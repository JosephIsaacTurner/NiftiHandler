...
from .NiftiHandlerCustom import NiftiHandlerDjangoS3
...
class AtlasLabeler:
    ...
    def __init__(self, nifti, atlas=os.path.join(atlases_dir,"HarvardOxford-cort-maxprob-thr0-2mm.nii.gz"), min_threshold=1, max_threshold=1):
        ...
        if type(nifti) in [NiftiHandler, NiftiHandlerDjangoS3]:
            nifti = nifti.to_nifti_obj()
        elif type(nifti) != nib.nifti1.Nifti1Image:
...
class MultiAtlasLabeler:
    ...
    def __init__(self, nifti_path, atlas_list=os.path.join(atlases_dir,'harvoxf_atlas_list.csv'), min_threshold=1, max_threshold=1):
        ...
        if type(nifti_path) in [NiftiHandler, NiftiHandlerDjangoS3]:
            nifti = nifti_path.to_nifti_obj()
        elif not isinstance(nifti_path, nib.nifti1.Nifti1Image):
            try:
...
class CustomAtlasLabeler:
    ...
    def __init__(self, nifti_path, min_threshold=1, max_threshold=1, atlas=os.path.join(atlases_dir, "joseph_custom_atlas.csv")):
...
def label_csv_multi_atlas(csv_path, atlases=[os.path.join(atlases_dir,"HarvardOxford-cort-maxprob-thr0-2mm.nii.gz"), os.path.join(atlases_dir,"HarvardOxford-sub-maxprob-thr0-2mm.nii.gz")], roi_col_name="orig_roi_vol", min_threshold=1, max_threshold=1):
    """
